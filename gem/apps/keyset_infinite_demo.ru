...
class PetsController < ActionController::Base # :nodoc:
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.turbo_stream {render inline: index_turbo_stream(@pets, @pagy)}
    end
  ...
  <html lang="en">
    ...
    <body>
      ...
      <div class="content">
        ...
        <h3>Collection</h3>                        
        <div id="pets"> </div>
        <%= turbo_frame_tag "pagination", src: pets_path(format: :turbo_stream), loading: :lazy %>
      </div>
...
def index_turbo_stream(pets, pagy)
  turbo_stream_output = <<~ERB
   <%= turbo_stream.append "posts" do %>
     <%= render pets_table(pets) %>
   <% end %>
  <% if pagy.next.present? %>
    <%= turbo_stream.replace "pagy" do %>
      <%= turbo_frame_tag "pagy", src: pets_path(page: pagy.next, format: :turbo_stream), loading: :lazy %>
    <% end %>
  <% end %>
  ERB
end
