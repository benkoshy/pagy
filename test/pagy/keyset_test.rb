...
describe 'Pagy::Keyset::ActiveRecord' do
  describe '#initialize' do
    it 'raises ArgumentError without arguments' do
      err = assert_raises(ArgumentError) { Pagy::Keyset::ActiveRecord.new }
      assert_match(/wrong number of arguments/, err.message)
    end
    it 'raises ArgumentError without arguments' do
      err = assert_raises(Pagy::InternalError) { Pagy::Keyset::ActiveRecord.new(Pet.all) }
      assert_match(/the set must be ordered/, err.message)
    end
    it 'is an instance of Pagy::Keyset::ActiveRecord' do
      _(Pagy::Keyset::ActiveRecord.new(Pet.order(:id))).must_be_instance_of Pagy::Keyset::ActiveRecord
    end
    it 'raises Pagy::InternalError for inconsistent page/keyset' do
      ...
      err = assert_raises(Pagy::InternalError) do
        Pagy::Keyset::ActiveRecord.new(Pet.order(:id), items: 10, page: page_animal_id)
      end
  ...
  describe '#setup_order' do
    it 'extracts the scope order' do
      pagy = Pagy::Keyset::ActiveRecord.new(Pet.order(:id))
      _(pagy.instance_variable_get(:@keyset)).must_equal({:id => :asc})
      pagy = Pagy::Keyset::ActiveRecord.new(Pet.order(id: :desc))
      _(pagy.instance_variable_get(:@keyset)).must_equal({:id => :desc})
      pagy = Pagy::Keyset::ActiveRecord.new(Pet.order(:id, animal: :desc))
  ...
  describe 'handles the page/cursor' do
    it 'handles the page/cursor for the first page' do
      pagy = Pagy::Keyset::ActiveRecord.new(Pet.order(:id), items: 10)
    ...
    it 'handles the page/cursor for the second page' do
      pagy = Pagy::Keyset::ActiveRecord.new(Pet.order(:id), items: 10, page: "eyJpZCI6MTB9")
    ...
    it 'handles the page/cursor for the last page' do
      pagy = Pagy::Keyset::ActiveRecord.new(Pet.order(:id), items: 10, page: "eyJpZCI6NDB9")
